...
#' @examples
#' QM(obs, mod, var)
#' QM(obs, mod, var, frq = "A")
#' QM(obs, mod, var, frq = "M")
#' QM(obs, mod, var, frq = "M", pp_threshold = 0.1)
#' QM(obs, mod, var, frq = "M", pp_factor = 1 / 1000)
#' QM(obs, mod, var, frq = "M", pp_threshold = 0.1, pp_factor = 1 / 1000)
#'
#' @export
QM <- function(obs, mod, var, frq = "M", pp_threshold = 1, pp_factor = 1e-2) {
  ...
  PDF_mod <- getDist(
    matrix(mod_series[, 1:y_obs], nrow = dim(mod_series)[1]),
    mu_mod, std_mod, skew_mod, skewy_mod, var
  )
...
QDM <- function(obs, mod, var, frq = "M", pp_threshold = 1, pp_factor = 1e-2, rel_change_th = 2, inv_mod_th = pp_threshold) {
  # 1) Format inputs and get statistics of the observed and modeled series of
  #    the historical period (formatQM function of the climQMBC package).
  format_list <- formatQM(obs, mod, var, frq, pp_threshold, pp_factor)
