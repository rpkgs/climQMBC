...
#' - Quantile Mapping (`QM`):
#'
#' Proposed by Cannon et al. (2015). Correction is performed to monthly or
#' annual precipitation or temperature data in a single location. An independent
#' probability distribution function is assigned to each month and to each
#' projected period based on the Kolmogorov-Smirnov test. If annual frequency is
...
#' precipitation) corrected by the QM method. If monthly frequency is specified,
#' the length of this vector is 12 times the number of observed years `[12 x
#' nyear_mod, 1]`. If annual frequency is specified, the length of this vector is
#' equal to the number of observed  years `[nyear_mod, 1]`.
#'
#' @references
#'
...
#' specified, this is applied to the complete period. Each projected period
#' considers a window whose length is equal to the number of years of the
#' historical period and ends in the analyzed year. Correction of the historical
#' period is performed by the Quantile Mapping (QM) method, as described by
#' Cannon et al. (2015).
#'
#' @rdname QM
#' 
...
#' (Equation 4 of Cannon et al. (2015)) when the denominator is below
#' inv_mod_th.
#'
#' @param inv_mod_th (Optional) A float indicating the upper threshold of the
#' denominator of the scaling factor (Equation 4 of Cannon et al. (2015)) to
#' truncate the scaling factor. This parameter is defined as default as the
#' pp_threshold parameter, described above.
#'
#' @rdname QM
#'
#' @export
QDM <- function(obs, mod, var, frq = "M", pp_threshold = 1, pp_factor = 1e-2,
...
#'
#' Proposed by Chadwick et al.  (2021). Correction is performed to monthly or
#' annual precipitation or temperature data in a single location. An independent
#' probability distribution function is assigned to each month and to each
#' projected period based on the Kolmogorov-Smirnov test. If annual frequency is
#' specified, this is applied to the complete period. Each projected period
...
#'
#' @references
#' 2.Chadwick et al. (2022) `[under revision]`
#'
#' @rdname QM
#' @export
UQM <- function(obs, mod, var, frq = "M", pp_threshold = 1, pp_factor = 1e-2) {
  ...
  #    obtained in 4b) (getCDFinv). Equation
...
#' @description
#' - Scaled Distribution Mapping (`SQM`):
#'
#' Proposed by Switanek et al. (2017). Correction is performed to monthly or
#' annual precipitation or temperature data in a single location. The historical
#' period of the modeled series is bias corrected as a scenario where the
#' complete series is replaced by the bias corrected series. On the other hand,
#' for each year after the historical period the method considers a projected
#' period as a window whose length is equal to the number of years of the
#' historical period and ends in the analyzed year. From the bias corrected
#' series, only the last year is saved and all the others are discarded. This
#' approach aims to build a continuum for the bias corrected series.
#'
#' @references
#'
...
#'
#' @export
SDM <- function(obs, mod, var, frq = "M", pp_threshold = 1, pp_factor = 1e-2) {
  ...
  SDM <- matrix(0, dim(mod_series)[1], ny_mod - ny_obs)
